...
check(
	"ec past EOL single",
	{ trim(mkbuf { "foo" }, 0, 0, 0, 5) },
	{ 0, 0, 0, 3 }
)
check(
	"ec past EOL multi",
	{ trim(mkbuf { "foo", "bar" }, 0, 0, 0, 9) },
	{ 0, 0, 0, 3 }
)
check("zerowidth row0", { trim(mkbuf { "foo", "bar" }, 0, 0, 0, 0) }, nil)
check(
	"ec0 to prev line",
	{ trim(mkbuf { "foo", "bar" }, 0, 0, 1, 0) },
	{ 0, 0, 0, 3 }
)
check(
	"ec0 skips empty",
	{ trim(mkbuf { "foo", "", "bar" }, 0, 0, 2, 0) },
	{ 0, 0, 0, 3 }
)
check("ec0 empty above -> nil", { trim(mkbuf { "", "bar" }, 0, 0, 1, 0) }, nil)
check(
	"leading indent",
	{ trim(mkbuf { "    foo" }, 0, 0, 0, 7) },
	{ 0, 4, 0, 7 }
)
...
check(
	"internal ws kept",
	{ trim(mkbuf { "foo bar baz" }, 0, 0, 0, 11) },
	{ 0, 0, 0, 11 }
)
...
check(
	"mid blank kept",
	{ trim(mkbuf { "foo", "", "bar" }, 0, 0, 2, 3) },
	{ 0, 0, 2, 3 }
)
check(
	"ideo space kept",
	{ trim(mkbuf { "\u{3000}x" }, 0, 0, 0, 4) },
	{ 0, 0, 0, 4 }
)
